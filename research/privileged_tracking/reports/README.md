...
* **Open items carried from the reviews (not fixed here).** (a) One garbage play in
  ...
  resolution, output schema), not driver runs. (e) Phase 2 is committed on `claude/soccer-betting-edge-29h9yo` (phase 1 in commit
  25e942d, phase 2 including this synthesis in 4334852); the processed tables and model
  bundles under `data/raw/privileged/` are not committed, so the reproduction section
  assumes they are rebuilt first.
